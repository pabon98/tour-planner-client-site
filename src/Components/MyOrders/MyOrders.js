...
const MyOrders = () => {
    ...
    useEffect(() => {
        const url = `https://tourplanner-server.onrender.com/myOrders/${user?.email}`;
    ...
    useEffect(() => {
        const url = `https://tourplanner-server.onrender.com/services`;
        fetch(url)
            .then(res => res.json())
            .then(data => {
                setServices(data);
    ...
    const handleCancelOrder = (id) => {
        ...
        if (proceed) {
            const url = `https://tourplanner-server.onrender.com/${id}`;
